...
For daily news articles, the scraper stores the publisher URL in `link`, `_source_url`, and `resolved_url` when available. The historical archive only keeps rows with a real non-Google publisher URL and a verified article date source (`original_feed` or `article_metadata`); date-only publisher metadata is allowed when it comes from the article page itself. Rows that only have fallback/estimated dates or unresolved Google News wrapper URLs are rejected during the daily archive merge.
