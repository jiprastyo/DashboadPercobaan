...
| BPS SDG variables | `bps-sdg-sakernas.ts` (`scrapeBPSSDGSakernas`, manual) | `data/bps/sdg-sakernas.json` | `getBPSSDGSakernasData()` | `/sdg` (+ legacy `/sdg-sakernas`) |
...
| BI PMI | `bi-pmi.ts` (`runBIPMI`) | `data/bi/pmi/series.json` (currently `[]`) | **none — UI shows `getSamplePMIData()` sample** | `/makro-indonesia`, `/` |
...
|---|---|---|---|
| `/` Ikhtisar | async server | `OverviewDashboard` (server!) | data via `src/lib/overview-data.ts:getOverviewDashboardData()` — mixes real + sample (see add-visualization) |
...
| `/makro-indonesia` | server | `MakroIndonesiaClient` (1113 lines) | TPT Sakernas timeline + comparison, IHK, trade, wisman, PMI (sample), PHK timeline (sample) |
...
| `/sdg` and `/sdg-sakernas` | server ×2 (duplicated) | `SDGSakernasClient` | nav points to `/sdg`; `/sdg-sakernas` is legacy; client ignores 2 of its 4 props |
