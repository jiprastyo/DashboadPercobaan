...
export const STALE_LIMIT_DAYS: Record<string, number> = {
  ...
  "bps-provinsi": 45,
  "bi-pmi": 45,
  // asean-nso / asean-fallback: World Bank / ASEAN NSO modeled series are
  // annual releases with an expected ~1 year publication lag -- the
  // data-catalog states this explicitly ("Expected lag ~1 year: comparator
  // countries capping at the previous year is normal source latency, not
  // staleness"). 400 days (~1 year + slack) matches the precedent already
  // set by getBenchmarkTargets()'s own 400-day window for the same reason.
  // 45 days was flagging a source that is behaving exactly as designed.
  "asean-nso": 400,
  "asean-fallback": 400,
  // asean-tiered (2026-09-16): tier 1 NSO (MY/SG/PH monthly-quarterly, lag ~1-2
  // months), tiers 2-3 ILO modeled (annual, ~1y lag). 400d matches the
  // fallback precedent; monthly CI runs refresh it.
  "asean-tiered": 400,
  // Manual-run, no cron: SDG Sakernas cards are regenerated by hand from
  // BPS Web API pulls, not a scheduled scraper. Window is generous
  // (quarterly/annual official releases) and keyed off the file's own
  // `_generated_at`, not a data/_metadata.json scrapers entry -- see
  // getManualSourceFreshness() in data-loader-server.ts.
  "bps-sdg-sakernas": 180,
};

// Fallback window (days) for a scraper id not listed above.
export const DEFAULT_STALE_LIMIT_DAYS = 14;

// Sources whose wrapped run can legitimately report `status: "partial"` in
// steady state, with no operator action available or needed --
// pipeline-debugging's failure-modes.md #8 ("Silent-success traps") documents
// both as sources whose per-item/per-country errors are normal, expected
// noise rather than a signal something broke:
//   - "google-trends-py": hardcoded ops metrics regardless of real
//     per-keyword outcomes; retries + backoff are already built in.
//   - "asean-nso": embeds per-country fetch errors in the data file itself
//     (NSO sites flap independently) while still writing whatever
//     succeeded; it also has no UI consumer ("none" in the data catalog),
//     so a warning badge here is never actionable by a reader anyway.
// evaluateFreshness() treats "partial" as "ok" (not "warning") ONLY for
// these ids, so the /operasional "Yang perlu dicek" panel and every public
// SourceFreshnessBadge stop false-alarming on routine, expected partials.
// This does NOT suppress staleness or error/failed statuses for these
// sources -- only the "last run was partial" branch below.
export const EXPECTED_PARTIAL = new Set<string>(["google-trends-py", "asean-nso"]);
 ...
 * (unless the source is in EXPECTED_PARTIAL, where partial is steady-state
 * and reported as ok); else ok. Pure function (no I/O) so it can be
 * imported by both the server-only data loader and a client component
 * without pulling in `fs`.
 */
export function evaluateFreshness(params: {
  ...
  if (normalizedStatus === "partial") {
    if (EXPECTED_PARTIAL.has(source)) {
      return { status: "ok", reason: `Segar, ${ageDays} hari sejak update (parsial steady-state, normal untuk sumber ini).` };
    }
