...
import { downloadCsv, type CsvSource } from '@/lib/csv-export';
...
interface CsvDownloadButtonProps {
  ...
  /** Optional provenance for this chart's data -- written as a `# Sumber: ...`
   * comment line inside the CSV itself (project-guardrails g extended to
   * exported files, not just on-screen attribution). */
  source?: CsvSource;
  className?: string;
}
...
export default function CsvDownloadButton({ filename, rows, source, className }: CsvDownloadButtonProps) {
  return (
    <button
      type="button"
      onClick={() => downloadCsv(`${filename}.csv`, rows, source)}
